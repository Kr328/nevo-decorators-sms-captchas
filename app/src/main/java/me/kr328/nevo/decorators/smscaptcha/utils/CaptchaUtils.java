...
public class CaptchaUtils {
    private static final Pattern defaultIdentifyPattern = Pattern.compile(".*((验证|认证|確認|驗證|校验|动态|确认|随机|激活|兑换|交易|授权|操作|提取|安全|登陆|登录|verification |confirmation )(码|碼|代码|代碼|号码|密码|code|コード)|口令|Steam|快递|快件|单号|订单).*");
    ...
    public CaptchaUtils(boolean useDefault ,Pattern customIdentifyPattern, Pattern customParsePattern) {
        ...
        this.customParsePattern    = customParsePattern;
    }
    ...
    public String[] findSmsCaptchas(String[] messages) {
        ...
        for ( String message : messages ) {
            ...
            while (customMatcher.find())
                captchas.add(customMatcher.group());

            while (useDefault&&defaultMatcher.find())
                captchas.add(defaultMatcher.group());
        }
